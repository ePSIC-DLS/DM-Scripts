...
// T. J. A. Slater, October 2018

// version:20190521, v0.3.0si
...
taggroup c_check, f_check, c_freqfield, c_stepfield, c_scansizefield, c_exposurefield, c_itfield, f_freqfield, frangefield, f_stepsizefield, f_scansizefield, f_exposurefield, framesfield
...
number fft_coeff_find(image img, number scansize, number mag)
	{
	/* 
	...
	*/
	...
	number cutoff = 2*1000*((2*10**8)/(mag*scansize))
	
	//showimage(img)
	
	image hanning_img
	hanning_img = CreateHanning(img)
	//showimage(hanning_img)
	...
	mask=tert(iradius<cutoff,0,mask) // Mask out central peak with mask to 0.5 nm (hopefully calibrated correctly)
	
	//ShowImage(mask)
	
	string mFields = "Area"      // Specify analysis results
	Number minParticleSize = 50                  // Limit accepted
	Number doLabel = 0                              // Add labeled mask to inputImage True/False

	imageDocument ResultsDoc = FindParticles(img,mask,mFields, 0, doLabel, foundParticlesImage)
	image x_img :=ImageDocumentGetImage(ResultsDoc,0)
	
	//Result("\n"+max(x_img))
	if(max(x_img)>minParticleSize)
		{
		//Result(max(x_img)
		imageDocument ResultsDoc2 = FindParticles(img,mask,mFields, minParticleSize, doLabel, foundParticlesImage)
		image x_img2 :=ImageDocumentGetImage(ResultsDoc2,0)
		//number imageid=x_img2.imagegetid()
		
		//ImageDocumentShow( ResultsDoc )

		number num_p, num_vals
		getsize(x_img2, num_vals, num_p)
...
number autofocus_fine(string finetype,number focusrange, number step_size, number scan_size, number exposure, number top, number left, number bottom, number right, number xsize, number ysize, image img, number mag)
	{
	/* 
	...
	*/
	...
	for(i = 0; i<max_it; i++)
		...
		//result("\n FFT " + i + ": ")
		if(finetype=="FFT")
			{coefficients[i,0] = fft_coeff_find(sub_img, scan_size, mag)
			}
...
number autoSI(number top, number left, number bottom, number right, number xsize, number ysize, string filename)
	{
	/* 
		WARNING: Untested
	...
	*/
	...
	image survey := SIGetSurveyImage() 

	if ( ImageIsValid( survey ) )
		{
		DeleteImage( survey )        // Closing the survey image will revert the STEM-SI palette to selection state
		}
	...
	//workspacesaveas(wsid, filename)
	
	//NOte to self: Should I just try to save each data set rather than saving the workspace?
	
	//Clean image documents so they don't ask to be saved
	number im_docs = countimagedocuments()
	for(number n=0;n<im_docs;n++)
		{
		imagedocument im_doc = getimagedocument(n)
		imagedocumentclean(im_doc)
		}
...
number autostem(taggroup stagelims, taggroup imsettings, taggroup focussettings)
	...
	*/
	...
	number lowmag, highmag, scansize_low, scansize_high, SI_acquire, frames
	...
	imsettings.taggroupgettagasnumber("SI_acquire", SI_acquire)
	imsettings.taggroupgettagasnumber("frames", frames)
	...
	//Set Survey Image for SI if SI is selected
	/*if(SI_acquire==1)
		{
		image survey := SIGetSurveyImage() 
				...
				for (j=0; j<(HighMag/LowMag); j++)
					...
					if(flag == 1)
						...
						if(n%f_freq==0)
							...
							if(f_check==1) autofocus_fine(f_type, frange, f_stepsize, f_scansize, f_exposure, top, left, bottom, right, xsize_high, ysize_high, img, highmag)
						...
						if(frames==1)
						{
						string filename = folder+"autoSTEM_"+im_count
						...
						if(frames>1)
						{
							...
							for(number f=0;f<frames;f++)
							{
								string filename = folder+"autoSTEM_"+im_count+"_"+f
						...
						if(SI_acquire==1)
							{
							string filename = folder+"autoSTEM_"+im_count
							string si_filename = filename+"_SI"
							autoSI(top,left,bottom,right,256,256,si_filename)
							}
	...
	TagGroup CreateFocusSettingsDialog(object self) 
		{
			...
			c_itfield = DLGCreateRealField(10, 12, 1);
			...
			f_scansizefield = DLGCreateRealField(1024, 12, 1);
	...
	TagGroup CreateImageSettingsDialog(object self) 
		{
			...
			lexposurefield = DLGCreateRealField(5, 12, 1);
			...
			hexposurefield = DLGCreateRealField(20, 12, 1);
			hexposure.dlgaddelement(hexposurefield)
			hbox_items.dlgaddelement(hexposure)
			
			taggroup frames=DLGCreateBox("No. of frames")
			framesfield = DLGCreateRealField(1, 12, 1);
			frames.dlgaddelement(framesfield)
			hbox_items.dlgaddelement(frames)
...
class CreateAutoImDialog : uiframe
	...
	TagGroup CreateImTagGroup(object self)
		...
		index = imtags.TagGroupCreateNewLabeledTag( "frames" ) 
		imtags.TagGroupSetIndexedTagAsNumber( index, dlggetvalue(framesfield) )
		index = imtags.TagGroupCreateNewLabeledTag( "path" ) 
		imtags.TagGroupSetIndexedTagAsString( index, dlggetstringvalue(pathfield) )
		index = imtags.TagGroupCreateNewLabeledTag( "SI_acquire" ) 
		self.dlggetvalue("si_check",si_on)
		imtags.TagGroupSetIndexedTagAsNumber( index, si_on )
	...
	TagGroup CreateDialog(object self)
		...
		taggroup si_check = DLGCreateCheckBox("Acquire SI",0)
		si_check.dlgidentifier("si_check")
		dialog.dlgaddelement(si_check)
